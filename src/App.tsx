...
const LazyModels3dPage = lazy(() =>
  import("./pages/Models3dPage").then((module) => ({
    default: module.Models3dPage,
  })),
);
...
function App() {
  ...
  const [currentPage, setCurrentPage] = useState<AppPageId>(() =>
    readJson(STORAGE_KEYS.page, "dashboard"),
  );
  const [profile, setProfile] = useState<UserProfile>(() =>
    readJson(STORAGE_KEYS.profile, defaultProfile),
  );
  const [settings, setSettings] = useState<SystemSettings>(() =>
    readJson(STORAGE_KEYS.settings, defaultSettings),
  );
  const [selectedChat, setSelectedChat] = useState<ChatThreadId>(() =>
    readJson(STORAGE_KEYS.chatThread, "bot"),
  );
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>(() =>
    readJson(STORAGE_KEYS.chatMessages, defaultMessages),
  );
