...
export function AppShell({ currentPage, pageTitle, profile, onNavigate, onBackToLanding, children }: AppShellProps) {
  ...
  return (
    <div className="min-h-screen bg-[#1e1e2f] text-[#f8fafc]">
      <div className="flex min-h-screen">
        ...
        <aside className={cn('fixed inset-y-0 left-0 z-50 w-72 flex-col border-r border-white/5 bg-[#222233] transition-transform duration-200 xl:static xl:z-auto xl:flex xl:w-64', isMobileMenuOpen ? 'flex translate-x-0' : '-translate-x-full xl:translate-x-0 xl:flex')}>
          <button
            type="button"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[18px] font-bold tracking-[-0.5px] text-white">Agro Control</div>
                  ...
                  {navigation
                    ...
                    .map((item) => (
                      ...
                      >
                        ...
                        <i className={cn('fas', item.icon, 'w-[18px] text-center text-[13px]', currentPage === item.id ? 'opacity-100' : 'opacity-60')} />
                        <span className="flex-1">{item.label}</span>
                        {item.badge ? (
                          <span className={cn('ml-auto min-w-[18px] rounded-full px-1.5 py-0.5 text-center font-mono text-[9px] font-bold', badgeTone(item.badge))}>
                            {item.badge}
                          </span>
                        ) : null}
                      </button>
          ...
          <div className="flex items-center gap-3 border-t border-white/5 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="truncate text-[10px] text-slate-500">Administrador</div>
            </div>
            <button
              type="button"
        ...
        <main className="flex min-h-screen flex-1 flex-col bg-[#1e1e2f] xl:ml-0">
          <header className="sticky top-0 z-20 border-b border-white/5 bg-[#1e1e2f]/95 px-4 py-[14px] backdrop-blur md:px-7">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h1 className="text-[16px] font-bold tracking-[-0.3px] text-white md:text-[18px]">{pageTitle}</h1>
                <p className="mt-0.5 font-mono text-[11px] text-slate-500">Agro Control / {pageTitle}</p>
              </div>
              ...
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <button className="flex h-9 w-9 items-center justify-center rounded-[8px] border border-white/10 bg-[#27293d] text-slate-300 transition hover:border-white/20 hover:text-emerald-300" type="button">
          ...
          <section className="flex-1 overflow-y-auto px-4 py-5 md:px-6 md:py-6">{children}</section>
        </main>
      </div>
    </div>
  );
